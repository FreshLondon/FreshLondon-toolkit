	...
	if (window.location.href.indexOf("ct_builder") > -1) {
		...
		jQuery(window).scroll(function (event) {
			...
			if (scrollPosition < section1start + (0.5 * section1Height)) {
				...
				//console.log((spiderMaxTranslate / 100) * getAnimPercSpider(0, section1start + (0.5 * section1Height), scrollPosition));
				//sectionSpider.css({transform: 'translateY(' + -0.16 * getAnimPercSpider(0, section1start + (0.5 * section1Height), scrollPosition) + 'vh)'});
				sectionSpider.css({transform: 'translateY(-' + (spiderMaxTranslate / 100) * getAnimPercSpider(0, section1start + (0.5 * section1Height), scrollPosition) + 'px)'});
				//vehicle1.css({transform: 'translateY(' + -0.31 * getAnimPercSpider(0, section1start + (0.5 * section1Height), scrollPosition) + 'vh)'});
				vehicle1.css({transform: 'translateY(-' + (spiderMaxTranslate / 100) * getAnimPercSpider(0, section1start + (0.5 * section1Height), scrollPosition) + 'px)'});
				vehicle1purple.css({transform: 'translateY(' + (spiderMaxTranslate / 100) * getAnimPercSpider(0, section1start + (0.5 * section1Height), scrollPosition) + 'px)'});
				vehicle3.css({transform: 'translateX(100vw)'});
			}
			...
			if (scrollPosition >= section8start - (windowHeight * .75)) {
				sectionSpider.css('opacity', '0');
			} else if (scrollPosition >= (section3start - (windowHeight * .75))) {
			...
			if (jQuery(vehicle1).offset().top + (1.1 * jQuery(vehicle1).height()) > vehicle1RotateTo + RotateMobile && jQuery(vehicle1).offset().top + (1.1 * jQuery(vehicle1).height()) < section5start + (0.3 * section5Height) + RotateMobile) { // ensure van turn is 90 degrees and centered
				jQuery(vehicle1).css('transition', 'ease transform 0.1s');
				jQuery(vehicle1).css({transform: 'translate(0px, -' + vehicle1MaxTranslate + 'px) rotate(90deg)'});
				vehicle3.css({transform: 'translateX(100vw)'});
			...
			if (jQuery(vehicle1).offset().top + (1.1 * jQuery(vehicle1).height()) > section5start + (0.3 * section5Height) + RotateMobile) { // ensure full exit
				jQuery(vehicle1).css({transform: 'translateX(' + (-80 - RotateMobile) + 'vw) translateY(-' + vehicle1MaxTranslate + 'px) rotate(90deg)'});
				vehicle3.css({transform: 'translateX(100vw)'});
			}
