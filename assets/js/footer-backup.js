...
function getDegrees (startpoint, endpoint) {
	...
	return Math.min(90, (0.9 * (((travelDistance - travelLeft) / travelDistance) * 100)));
}
...
function getColour (startpoint, endpoint) {
	...
	travelLeft = (parseFloat(endpoint) - parseFloat(jQuery(window).scrollTop()));
	//console.log( Math.max(0,((travelDistance-travelLeft)*parseFloat(changePerPixel))));
...
jQuery(document).ready(function () {
	if (window.location.href.indexOf("ct_builder") > -1) {
		...
		jQuery(window).scroll(function (event) {
			...
			console.log('scroll positition is: ' + (scrollPosition + windowHeight));
			...
			if (scrollPosition >= vehicle1trans && scrollPosition <= vehicle1trans + vehicle1.height()) { // vehicle1 purple fade in
				vehicle1purple.css('opacity', getColour(vehicle1trans, vehicle1trans + vehicle1.height()));
				vehicle1.css('opacity', 1 - getColour(vehicle1trans, vehicle1trans + vehicle1.height()));
			}
			if (scrollPosition >= vehicle1trans + vehicle1.height() && scrollPosition <= vehicle1trans + 2 * vehicle1.height()) { // vehicle1 purple fade out
				vehicle1purple.css('opacity', 1 - getColour(vehicle1trans + vehicle1.height(), vehicle1trans + 2 * vehicle1.height()));
				vehicle1.css('opacity', getColour(vehicle1trans + vehicle1.height(), vehicle1trans + 2 * vehicle1.height()));
			}
			...
			if (scrollPosition + windowHeight > vehicle1RotateTo + (0.5 * jQuery('#section4circle').height()) && scrollPosition + windowHeight < section5start){ // ensure van turn is 90 degrees and centered
				jQuery(vehicle1).css({transform: 'rotate(90deg) translateX('+(0.5*jQuery(vehicle1).width())+')'});
			}
			
			if (scrollPosition + windowHeight >= section5start  && scrollPosition + windowHeight <= section5start + (0.1 * section5Height)) { // vehicle1 exit
				
				jQuery(vehicle1).css({transform: 'rotate(90deg) translate('+(-0.25*jQuery(vehicle1).width())+'px,' + -getDriveOnOffPos(section5start,section5start + (0.1 * section5Height)) + 'vw) '});
			}
			
			if(scrollPosition + windowHeight > section5start + (0.1 * section5Height)){ // ensure full exit
				jQuery(vehicle1).css({transform: 'translateX(-80vw) rotate(90deg)'});
			}
			
			if (scrollPosition + windowHeight >= section5start + (0.3 * section5Height) && scrollPosition + windowHeight <= section5start + (0.5 * section5Height)) { //vehicle2 enter
				...
				jQuery(vehicle2).css({transform: 'translateX(' + (-80 - getDriveOnOffPos(section5start + (0.3 * section5Height), section5start + (0.5 * section5Height))) + 'vw)'});
			...
			if (scrollPosition + windowHeight >= section6start  && scrollPosition + windowHeight <= jQuery('#image-345-9').offset().top + jQuery('#image-345-9').height()) { // vehicle 2 exit, vehicle2blue enter
				...
				jQuery(vehicle2blue).css({transform: 'translateX(' + (  (((0.5*jQuery(vehicle2).width())/jQuery(window).width())*100) -80 - getDriveOnOffPos(section6start, jQuery('#image-345-9').offset().top + jQuery('#image-345-9').height())) + 'vw)'});
				jQuery(vehicle2).css({transform: 'translateX(' + (-getDriveOnOffPos(section6start, jQuery('#image-345-9').offset().top)) + 'vw)'});
